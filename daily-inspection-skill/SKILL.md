...
- AI 部分：AI 脚本负责生成 `AI-inspection/out/non_deep_users_YYYY-MM-DD.json`，总编排从当天源 JSON 派生非深度用户名单；如果已有 `AI-inspection/out/non_deep_user_names_YYYY-MM-DD.json`，可作为兜底读取。
...
| AI 巡检 | `AI-inspection/out/non_deep_users_YYYY-MM-DD.json` | 直接读当天源 JSON 并派生名单；已有 `non_deep_user_names_YYYY-MM-DD.json` 时可兜底 |
